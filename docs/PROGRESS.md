...
**Repo root cleanup (same branch).** AI tooling untracked and gitignored —
`.agents/` (69 files, more than all of `src/`), `.windsurf/`, `.claude/` (symlinks
into `.agents/`, meaningless in a clone) and `skills-lock.json`. All still on disk,
just no longer part of the source of record. `todo.md` deleted (a finished Phase C
checklist, fully superseded by this file; git history keeps it). **This file moved
to `docs/PROGRESS.md`** so the README is the first thing a reviewer meets —
references updated in `CLAUDE.md` and `src/storefronts/registry.ts`. `CLAUDE.md`
and `AGENTS.md` deliberately KEPT at root (`AGENTS.md` is regenerated by `next dev`,
so untracking it would just create permanent churn). Tracked files 194 → ~110.

**NOT done / next:** not deployed yet (no Vercel project, no prod Neon DB, no live
URL) — steps are in README "Deploying". No screenshots in the README. Remaining
security gaps are listed at the end of `security.md` (no RLS, no session
revocation, no audit log, no CSP, no checkout throttling).
